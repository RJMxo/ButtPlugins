...
class Calculation {
    ...
    fun randomDelay(weightedDistribution: Boolean, min: Int, max: Int, deviation: Int, target: Int): Long {
        return if (weightedDistribution) {
            ...
            clamp(-Math.log(Math.abs(random.nextGaussian())) * deviation + target, min, max).toLong()
        } else {
            /* generate a normal even distribution random */
            clamp(Math.round(random.nextGaussian() * deviation + target).toDouble(), min, max).toLong()
        }
    }
